...
- **DataTable** ([DataTable/DataTable.md](../DataTable/DataTable.md))
  ...
  3. `buildState.js` の `set()` ヘルパーで `>` が `>>`(ビットシフト)の意図だったと見られるタイポにより、配列判定分岐が常に false になり、数値パスセグメントが配列にならない(検証済み・実在バグ)。
  4. ピン留め列のオフセット計算が、先行するピン留め列の幅がまだ取得されていないタイミングで例外になりうる(要検証)。
